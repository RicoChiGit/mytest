...
var ___cxa_throw = function() {
 ...
 return function() {
  if(Module.IsWxGame);console.log("Exception at: \n" + stackTrace());
...
function initRuntime() {
 ...
 _WXFSInit(unityNamespace.ttlAssetBundle?unityNamespace.ttlAssetBundle:5);TTY.init();
...
function jsAudioCreateChannel(callback, userData) {
 ...
 channel.playSoundClip = function(soundClip, startTime, startOffset) {
  try {
   ...
   err("Channel.playSoundClip error. Exception: " + e);
  }
 };
...
function _JS_Sound_Play(bufferInstance, channelInstance, offset, delay) {
 ...
 try {
  ...
  err("playSoundClip error. Exception: " + e);
 }
}
...
function _JS_Sound_SetPitch(channelInstance, v) {
 ...
 try {
  ...
  err("JS_Sound_SetPitch(channel=" + channelInstance + ", pitch=" + v + ") threw an exception: " + e);
 }
}
...
function _JS_Sound_SetVolume(channelInstance, v) {
 ...
 try {
  ...
  err("JS_Sound_SetVolume(channel=" + channelInstance + ", volume=" + v + ") threw an exception: " + e);
 }
}
...
function _JS_WebRequest_Create(url, method) {
 ...
 if(GameGlobal.TEXTURE_PARALLEL_BUNDLE){GameGlobal.ParalleLDownloadTexture(_url)}var http = new GameGlobal.unityNamespace.UnityLoader.UnityCache.XMLHttpRequest();http.open(_method, _url, true);
...
function _reTryRequest(request,arg,onresponse,needRetry){var http=wr.requestInstances[request];http.retryCount = http.retryCount || 0;http.retryCount++;var xml=new XMLHttpRequest();xml.open('GET',http.paramsCache.url,true);xml.responseType=http.responseType;xml.onload=function(){if(http.status>=400&&needRetry){setTimeout(function(){_reTryRequest(request,arg,onresponse)},1000);return false;}else{if(onresponse){var kWebRequestOK=0;var byteArray=new Uint8Array(xml.response);if(byteArray.length!=0){if(WXFS.isWXAssetBundle(http.paramsCache.url)){var arrayBuffer=new Uint8Array(http.response).buffer;let path=WXFS.url2path(http.paramsCache.url);let numberfd=WXFS.path2fd.get(path);if(numberfd==undefined){numberfd=WXFS.newfd();WXFS.path2fd.set(path,numberfd);}let wxStream=WXFS.fd2wxStream.get(numberfd);if(wxStream==undefined){wxStream={node:{mode:32768,usedBytes:byteArray.length},fd:numberfd,path:path,seekable:true,position:0,stream_ops:MEMFS.stream_ops,ungotten:[],error:false};wxStream.stream_ops.read=WXFS.read;WXFS.fd2wxStream.set(numberfd,wxStream);}WXFS.cache.put(numberfd,arrayBuffer,http.isReadFromCache);dynCall("viiiiii",onresponse,[arg,http.status,0,0,0,kWebRequestOK]);path=unityNamespace.PathInFileOS(path);WXFS.disk.set(path,byteArray.length);return}var buffer=_malloc(byteArray.length);HEAPU8.set(byteArray,buffer);dynCall('viiiiii',onresponse,[arg,xml.status,buffer,byteArray.length,0,kWebRequestOK])}else{dynCall('viiiiii',onresponse,[arg,xml.status,0,0,0,kWebRequestOK])}}}};xml.send(http.postData);xml.onerror=http.onerror;xml.ontimeout=http.ontimeout;xml.onabort=http.onabort;console.error('load url error:' + http.paramsCache.url);GameGlobal.logmanager.warn('load url error:'+http.paramsCache.url);GameGlobal.realtimeLogManager.error('load url error:'+http.paramsCache.url);} function _JS_WebRequest_SetResponseHandler(request, arg, onresponse) {
 var http = wr.requestInstances[request];
 http.onload = function http_onload(e){if(http.status>=400&&http.paramsCache.method==='GET'&&/\b(settings|catalog)\.json\b/.test(http.paramsCache.url)){return _reTryRequest(request,arg,onresponse,true)}
  if (onresponse) {
   var kWebRequestOK = 0;
   var byteArray = new Uint8Array(http.response);
   if(byteArray.length!=0){if(WXFS.isWXAssetBundle(http.paramsCache.url)){var arrayBuffer=new Uint8Array(http.response).buffer;let path=WXFS.url2path(http.paramsCache.url);let numberfd=WXFS.path2fd.get(path);if(numberfd==undefined){numberfd=WXFS.newfd();WXFS.path2fd.set(path,numberfd);}let wxStream=WXFS.fd2wxStream.get(numberfd);if(wxStream==undefined){wxStream={node:{mode:32768,usedBytes:byteArray.length},fd:numberfd,path:path,seekable:true,position:0,stream_ops:MEMFS.stream_ops,ungotten:[],error:false};wxStream.stream_ops.read=WXFS.read;WXFS.fd2wxStream.set(numberfd,wxStream);}WXFS.cache.put(numberfd,arrayBuffer,http.isReadFromCache);dynCall("viiiiii",onresponse,[arg,http.status,0,0,0,kWebRequestOK]);path=unityNamespace.PathInFileOS(path);WXFS.disk.set(path,byteArray.length);return}
 ...
 http.onsave=function http_onsave(e){WXFS.cache.cleanable(WXFS.path2fd.get(e))};function HandleError(err, code){ http.retryCount = http.retryCount || 0; if (typeof http !='undefined' && http.paramsCache.method === 'GET' && /\b(settings|catalog)\.json\b/.test(http.paramsCache.url) && http.retryCount<2) {return setTimeout(function () {_reTryRequest(request, arg, onresponse)}, 1000);}
  if (onresponse) {
...
var SYSCALLS = {
 ...
 getStr: function(ptr) {
  var ret=UTF8ToString(ptr);if(WXFS.isWXAssetBundle(ret)){return WXFS.url2path(ret);}
  return ret;
 },
...
function ___sys_access(path, amode) {
 try {
  ...
  if(WXFS.isWXAssetBundle(path)){if(WXFS.path2fd.has(path))return 0;return WXFS.doWXAccess(path,amode)}return SYSCALLS.doAccess(path,amode);
 } catch (e) {
  if (typeof FS === "undefined" || !(e instanceof FS.ErrnoError)) abort(e);
  return -e.errno;
 }
...
function ___sys_open(path, flags, varargs) {
 SYSCALLS.varargs = varargs;
 try {
  var pathname=SYSCALLS.getStr(path);if(WXFS.isWXAssetBundle(pathname)){var numberfd=WXFS.path2fd.get(pathname);if(numberfd!==undefined){return numberfd;}const res=WXFS.fs.readFileSync(pathname);numberfd=WXFS.newfd();let wxStream={fd:numberfd,path:pathname,flags:flags,seekable:true,position:0,stream_ops:MEMFS.stream_ops,ungotten:[],node:{mode:32768,usedBytes:new Uint8Array(res).length},error:false,};wxStream.stream_ops.read=WXFS.read;WXFS.path2fd.set(pathname,numberfd);WXFS.fd2wxStream.set(numberfd,wxStream);WXFS.cache.put(numberfd,res);return numberfd}
...
function ___sys_stat64(path, buf) {
 try {
  path = SYSCALLS.getStr(path);
  if(WXFS.isWXAssetBundle(path)){return SYSCALLS.doStat(WXFS.wxstat,path,buf);}return SYSCALLS.doStat(FS.stat,path,buf);
 } catch (e) {
  if (typeof FS === "undefined" || !(e instanceof FS.ErrnoError)) abort(e);
  return -e.errno;
 }
...
function _glClientWaitSync(sync, flags, timeoutLo, timeoutHi) {
 return GameGlobal.unityNamespace.isAndroid?0x911A:GLctx.clientWaitSync(GL.syncs[sync], flags, convertI32PairToI53(timeoutLo, timeoutHi));
}
...
function _glTexStorage2D(x0, x1, x2, x3, x4) {window._lastTexStorage2DParams = [x0, x1, x2, x3, x4];if(x2 == 36196 || x2 == 37492 || x2 == 37493){return;}
